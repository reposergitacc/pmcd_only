#!/usr/bin/env python3
"""
PMCD-Only Hierarchical Two-Stage Training (K-Fold)
===================================================
Two independently trained binary GradientFusionTransformer models:
  Stage 1: no_pain (0) vs pain (1|2) -- all windows
  Stage 2: moderate (1) vs severe (2) -- pain-present windows only

Combined via chain rule at inference. No PMED pretraining.

Modes:
- Default: 5 randomized outer folds, 3 seeds = 15 runs
- --fixed-folds: Original fixed patient assignment
- --fold-seed: Reproducible random folds
- --loso: Leave-One-Subject-Out (49 folds)
- --quick: Pipeline test
"""
...
from pmcd_only.train_utils import (
    ...
    select_validation_calibration, apply_calibration, calculate_metrics,
    fit_binary_stage,
)
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output_dir is None:
        args.output_dir = KAGGLE_WORKING_DIR / "pmcd_hierarchical_kfold" if KAGGLE_WORKING_DIR.is_dir() else Path("pmcd_hierarchical_kfold")
...
def fine_tune_hierarchical(target: PainData, seed: int, fold: int,
    ...
    s1_loaders = {
        "train": make_loader(WindowDataset(s1_data, train_idx, mean1, std1, augment=True), args.batch_size, True, device, args.loader_workers),
        "val": make_loader(WindowDataset(s1_data, val_idx, mean1, std1, return_index=True), args.batch_size, False, device, args.loader_workers),
    }
    ...
    s1_best_state, s1_best_score, s1_best_epoch = fit_binary_stage(s1_model, s1_loaders, s1_criterion, args, device)
    ...
    pain_val = val_idx[np.isin(target.y[val_idx], [1, 2])]
    if len(pain_train) == 0 or len(pain_val) == 0:
        raise RuntimeError(f"seed={seed} fold={fold}: no pain windows in train/val for stage 2")
    ...
    s2_loaders = {
        "train": make_loader(WindowDataset(s2_data, pain_train, mean2, std2, augment=True), args.batch_size, True, device, args.loader_workers),
        "val": make_loader(WindowDataset(s2_data, pain_val, mean2, std2, return_index=True), args.batch_size, False, device, args.loader_workers),
    }
    ...
    s2_best_state, s2_best_score, s2_best_epoch = fit_binary_stage(s2_model, s2_loaders, s2_criterion, args, device)
    s2_model.load_state_dict(s2_best_state)

    # ---- Combined evaluation (validation for calibration) ----
    val_eval = (
        make_loader(WindowDataset(target, val_idx, mean1, std1, return_index=True), args.batch_size, False, device, args.loader_workers),
        make_loader(WindowDataset(target, val_idx, mean2, std2, return_index=True), args.batch_size, False, device, args.loader_workers),
    )
    val_labels, val_probs, _ = predict_hierarchical_probabilities(s1_model, s2_model, *val_eval, device, tta=True)
    calibration = select_validation_calibration(val_labels, val_probs)
    ...
    test_preds = apply_calibration(test_probs, calibration)

    result = {
        "seed": seed, "fold": fold, "approach": "hierarchical_pmcd_only_tta_calibrated",
        "stage1_best_epoch": s1_best_epoch, "stage2_best_epoch": s2_best_epoch,
        "stage1_val_macro_f1": s1_best_score, "stage2_val_macro_f1": s2_best_score,
    ...
    checkpoint = {
        "stage1_state_dict": {k: v.cpu() for k, v in s1_best_state.items()},
        "stage2_state_dict": {k: v.cpu() for k, v in s2_best_state.items()},
    }
...
def main() -> int:
    ...
    print("DATA: PMCD only (hierarchical two-stage, no PMED)", flush=True)
    ...
    for seed in seeds:
        for fold in folds:
            ...
            result, preds, checkpoint, norm = fine_tune_hierarchical(
            ...
            }, model_dir / f"pmcd_hier_seed{seed}_fold{fold}.pt")
    ...
    model_desc = ("Hierarchical two-stage GradientFusionTransformer "
                  "(stage1: no_pain vs pain; stage2: moderate vs severe on pain-only) trained on PMCD only")
    ...
    run_label = f"PMCD-only hierarchical {eval_mode}"
