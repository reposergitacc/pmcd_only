#!/usr/bin/env python3
"""
PMCD-Only Single-Model Training (LOSO)
=======================================
Leave-One-Subject-Out evaluation of 3-class GradientFusionTransformer trained on PMCD only.

Default: LOSO (49 folds x seeds). Use --fixed-folds for 5 fixed folds instead.

Example:
    python train_single_loso.py --pmcd-dir /path/to/pmcd_np --output-dir results_loso --seeds 42
"""
...
from pmcd_only.train_utils import (
    ...
    select_validation_calibration_simple, apply_calibration, calculate_metrics,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pmcd-dir", type=Path, default=None,
                        help="Directory with PMCD X.npy, y.npy, subjects.npy")
    ...
    parser.add_argument("--quick", action="store_true", help="1 seed/fold, <=3 epochs")
    ...
    parser.add_argument("--loso", action="store_true", default=True,
                        help="Leave-One-Subject-Out (default)")
    args, unknown = parser.parse_known_args()
    if unknown:
        print(f"Ignoring unrecognized argv: {unknown}", flush=True)

    if args.output_dir is None:
        args.output_dir = KAGGLE_WORKING_DIR / "pmcd_single_loso" if KAGGLE_WORKING_DIR.is_dir() else Path("pmcd_single_loso")
...
def main() -> int:
    ...
    print("DATA: PMCD only (no PMED)", flush=True)
